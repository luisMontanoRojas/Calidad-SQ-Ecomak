...
import { AdmEditCategoriaComponent } from '../../Categorias/adm-edit-categoria/adm-edit-categoria.component';
import { AdmAddCategoriaComponent } from '../../Categorias/adm-add-categoria/adm-add-categoria.component';
...
export class AdmListCategoriasTrComponent implements OnInit {
  ...
  onOptions(id: number) {
    this.router.navigateByUrl(`/admin/categoriasTr/${id}`);
  }
  ...
  onDialogResponseEdit(subscribeEdit: Category) {
    ...
    this.data.find(s => s.id == subscribeEdit.id).productsSize = subscribeEdit.cantProducts;
  }
